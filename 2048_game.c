...
void print_game(const struct game *game)
{
        ...
        mvprintw(1,0,"Time: %.2f seconds", elapsed_time); //curse 라이브러리에 있는 print함수.. 그냥 printf쓰면 출력 위치 오류로 엄청난 오 류가 발생해서 1행0열에다가 프린트할수있도록도와준다.
...
void init_curses()
{
        ...
        initscr(); // curses init
        start_color();
        cbreak(); // pass Ctrl-C, etc. to the program
        noecho(); // prevent keypresses from being echoed
